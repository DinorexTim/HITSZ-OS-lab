...
void vmprint_recursive(pagetable_t pgtbl, int level, uint64 va) {
  // there are 2^9 = 512 PTEs in a page table.
  // 遍历一个页表页的PTE表项 
  // printf("page table %s");
  for(int i = 0; i < 512; i++){
    ...
    /* 判断PTE的Flag位，如果还有下一级页表(即当前是根页表或次页表)，
       则递归调用vmprint_recursive函数，否则打印当前PTE的相关信息 */
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){ 
      ...
      va += offset;
      vmprint_recursive((pagetable_t)child, level + 1, va); // 递归调用
      va -= offset;
    } else if(pte & PTE_V){ 
      // leaf PTE.
      printf("||");
...
void sync_pagetable(pagetable_t u_pagetable, pagetable_t k_pagetable) { 
  pagetable_t u_pa = (pagetable_t)PTE2PA(u_pagetable[0]);
  pagetable_t k_pa = (pagetable_t)PTE2PA(k_pagetable[0]);
  for (int i = 0; i < 96; i++){
    k_pa[i] = u_pa[i];  // 复制用户页表到内核页表
  }
}